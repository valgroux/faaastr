<section class="team">
    <div class="container--1200">
        <h2 class="h2">Travaille en équipe avec<br> nos experts</h2>

        <div class="team__content">

            <div class="team__content__bloc">
                <div class="team__content__bloc__img"></div>
                <div class="team__content__bloc__texte">
                    <h3 class="h3">Damien</h3>
                    <p class="team__content__bloc__texte__job">Co-fondateur</p>
                    <p class="team__content__bloc__texte__description">Automatise les tâches les plus longues et pénibles</p>

                    <div class="team__content__bloc__texte__reseaux">
                        <div class="team__content__bloc__texte__reseaux__fb"></div>
                        <div class="team__content__bloc__texte__reseaux__twitter"></div>
                    </div>
                </div>

            </div>

            <div class="team__content__bloc">
                <div class="team__content__bloc__img"></div>
                <div class="team__content__bloc__texte">
                    <h3 class="h3">Anthony</h3>
                    <p class="team__content__bloc__texte__job">Co-fondateur</p>
                    <p class="team__content__bloc__texte__description">Automatise les tâches les plus longues et pénibles</p>

                    <div class="team__content__bloc__texte__reseaux">
                        <div class="team__content__bloc__texte__reseaux__fb"></div>
                        <div class="team__content__bloc__texte__reseaux__twitter"></div>
                    </div>
                </div>
                
            </div>

        </div>

        <button class="cta">
            <div class="cta__icon cta__icon--arrow"></div>
            <a href="#">On fait connaissance ?</a>
        </button>

    </div>
</section>